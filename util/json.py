...
def json_encode(data, include=None, exclude=None):
		"""
		...
		if include is not None or exclude is not None:
			ret = clean_data(ret, include, exclude)
...
def to_json_response(data, include=None, exclude=None, func_name=None, use_iframe=False):
		"""
		...
		"""
		data = json_encode(data, include, exclude)
		#data = clean_data(data, include, exclude)
		...
		if False: #if settings.DOJO_SECURE_JSON: #Modified from original to avoid additional setting
				data = "{}&&\n" + data
...
#custom Robert
def clean_data(data, include=None, exclude=None):
	"""clean_data will strip a list_dict or dictionary down using include or exclude
	by: Robert!"""
	if include is not None and exclude is not None:
		raise Exception, 'Cannot have both "include" and "exclude"'

	if not isinstance(data, list) and not isinstance(data, dict):
		raise Exception, "data is not a subclass of list or dictionary"

	if include is not None:
		if isinstance(data, dict):
			new_data = {}
			for key in include:
				if key in data.keys(): # Added to verify that all included keys are actually part of dictionary
			...
			for list_item in data:
				for key in include:
					if key in list_item.keys(): # Added to verify that all included keys are actually part of dictionary
						temp_data[key] = list_item[key]
				new_data.append(temp_data.copy())
	else:
		if isinstance(data, dict):
			new_data = {}
...
def render_to_json(template, data):
